...
def load_train(img_rows, img_cols):
    ...
    files = glob.glob(trainDir + "*[0-9].tif")
    for fl in files:
        flbase = os.path.basename(fl)
        img = get_im_cv2(fl, img_rows, img_cols)
        X_train.append(img)
        X_train_id.append(flbase[:-4])
        mask_path = trainDir + flbase[:-4] + "_mask.tif"
...
def load_test(img_rows, img_cols):
    ...
    files = glob.glob(testDir + "*[0-9].tif")
    X_test = []
    X_test_id = []
    total = 0
    start_time = time.time()
...
def find_best_mask():
    files = glob.glob(trainDir + "*_mask.tif")
    if(not files):
        return 0
    ...
    print(overall_mask.shape,'overall_mask.shape')
    return overall_mask
